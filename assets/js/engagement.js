...
(() => {
  ...
  document.addEventListener("submit", async (event) => {
    ...
    const newsletter = window.LuxureatNewsletter;
    if (!newsletter?.ajaxUrl || !newsletter?.nonce) {
      feedback.textContent = copy.unavailable;
      return;
    }
    button.disabled = true;
    feedback.textContent = copy.working;
    const bot = await createLuxBotProof(newsletter.botChallenge);
    if (!bot) {
      feedback.textContent = copy.unavailable;
    ...
    data.set("nonce", newsletter.nonce);
    data.set("bot_challenge", newsletter.botChallenge);
    data.set("bot_nonce", bot.nonce);
    data.set("bot_proof", String(bot.proof));
    try {
      const response = await fetch(newsletter.ajaxUrl, {
        method: "POST",
